...
<title>Loveneesh Dhir | Growth, Partnerships &amp; Ecosystem</title>
<meta name="description" content="Loveneesh Dhir builds growth infrastructure, strategic partnerships, and developer programs for technology companies. 450,000 users in 72 hours. 140+ contributors across 25+ countries.">
<meta name="author" content="Loveneesh Dhir">
<meta name="theme-color" content="#14110F" id="themeColor">
<script>
/* Runs synchronously before first paint, so there is no flash of the wrong
   theme. Site defaults to dark regardless of OS preference; a saved manual
   choice (from the toggle in the nav) always wins on return visits. */
(function(){
  var t = 'dark';
  try {
...
<meta property="og:site_name" content="Loveneesh Dhir">
<meta property="og:image" content="https://loveneeshdhir.com/og-image.jpg">
<meta property="og:image:width" content="1200">
<meta property="og:image:height" content="630">
<meta property="og:image:alt" content="Loveneesh Dhir speaking on the main stage at FIL Bangalore">
<meta name="twitter:card" content="summary_large_image">
<meta name="twitter:site" content="@LoveneeshDhir">
<meta name="twitter:creator" content="@LoveneeshDhir">
...
<script type="application/ld+json">
{
  ...
  "knowsAbout":["Go-to-Market Strategy","Business Development","Ecosystem Growth","Partnerships","User Acquisition","Community-Led Growth","Developer Programs","Community Building"],
  "sameAs":[
...
/* Real bug, verified: toggling data-theme changes --rust (and other tokens)
   for 100+ elements at once. Any of them with transition:color/background/
   border-color can commit the WRONG final computed color and get stuck
   there indefinitely (confirmed reproducible: waited 2s+ on the live toggle,
   never self-corrected; only forcing transition:none fixed it). Root cause
   looks like the engine mishandling many simultaneous transitions sourced
   from one custom-property change. Fix: suppress all color/background/
   border transitions for the one frame the theme actually flips (JS adds
   this class right before, removes it right after via rAF), then normal
   hover/interaction transitions resume unaffected. */
...
.section{padding:var(--s11) 0;border-top:1px solid var(--line);position:relative;overflow:hidden}
.section>.wrap{position:relative;z-index:1}
/* faint chapter numeral, watermark-style, behind the section's real content.
   color-mix against --ink (not a hardcoded rgba white) so it stays proportionally
   faint in both themes instead of vanishing in light mode or blowing out in dark. */
.sec-num{
  position:absolute;top:var(--s8);right:var(--s7);
  font-family:var(--f-mono);font-size:clamp(64px,9vw,150px);
...
/* ---- work: accordion, every headline always visible, one body expanded ----
   Native <details>/<summary>: keyboard access and correct semantics come
   free. Everything (metric, tag, headline, body) lives inside <summary> so
   nothing is hidden by the browser's own open/closed mechanics; the [open]
   attribute only drives our own CSS (line-clamp on the body, chevron
   rotation). name="work-acc" makes the four mutually exclusive in browsers
   that support it (Chrome/Firefox/Safari, all current) and degrades to
   harmless independent accordions elsewhere, never breaks. */
.work-list{display:flex;flex-direction:column;gap:.75rem;margin-top:var(--s7)}
.work-item{
  border:1px solid var(--line);border-radius:var(--r-lg);
  background:var(--paper);overflow:hidden;
  transition:border-color .2s;
}
.work-item[open]{border-color:var(--line-2)}
.work-item summary{
  display:grid;grid-template-columns:200px minmax(0,1fr) 28px;gap:var(--s6);
  align-items:start;padding:var(--s6);
  cursor:pointer;list-style:none;
}
.w-main{min-width:0}
.work-item summary::-webkit-details-marker{display:none}
.work-item summary:hover{background:var(--paper-2)}
.w-metric{position:sticky;top:calc(var(--nav-h) + 2rem);align-self:start}
/* no transition on transform here: the native name= grouping toggles two
   <details> elements' [open] state in one tick, and animating transform
   across that simultaneous change made browsers cross-attribute which
   chevron was rotating to which element (verified: removing the transition
   fixes it). Color can still transition safely. */
.w-chev{
  display:grid;place-items:center;width:28px;height:28px;margin-top:.15rem;
  border:1px solid var(--line-2);border-radius:50%;color:var(--ink-3);
  transition:color .2s;
}
.w-chev svg{width:11px;height:11px}
.work-item[open] .w-chev{transform:rotate(45deg);color:var(--rust)}
.work-item .w-body{
  display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:2;
  line-clamp:2;overflow:hidden;
}
.work-item[open] .w-body{
  -webkit-line-clamp:unset;line-clamp:unset;overflow:visible;
  animation:workReveal .35s var(--ease);
}
@keyframes workReveal{from{opacity:.5}to{opacity:1}}
.w-n{
...
.work-item summary:hover .w-n{color:var(--rust)}
.w-unit{
...
/* ============ EDUCATION ============ */
.sub-label{margin-top:var(--s7);padding-top:var(--s6);border-top:1px solid var(--line)}
...
.fab{
  ...
  transition:opacity .3s,transform .3s var(--ease),visibility 0s linear .3s,border-color .2s,color .2s;
}
body.deep .fab{
  opacity:1;visibility:visible;transform:none;
  transition:opacity .3s,transform .3s var(--ease),visibility 0s,border-color .2s,color .2s;
}
...
@media (max-width:960px){
  ...
  .work-item summary{
    ...
    grid-template-areas:"metric chev" "main main";
    gap:.5rem var(--s6);
  }
  .w-metric{grid-area:metric;position:static;display:flex;align-items:baseline;gap:.75rem}
  .w-main{grid-area:main}
  .w-chev{grid-area:chev}
  .w-unit{margin-top:0}
...
@media print{
  ...
  /* :root,:root[data-theme] matches the specificity of :root[data-theme="dark"/"light"]
     (0,2,0 either way) and, being declared later, wins the tie, so the print
     palette applies no matter which theme a visitor had toggled on screen. */
  ...
  .g-cap,.exp-more,#praise,.portrait,.ct-btns,.sec-head .label::after,.sec-num{display:none!important}
  .r{opacity:1!important;transform:none!important}
  .exp-row[hidden]{display:grid!important}
  .section,#work,#skills,#vol{padding:.8rem 0;border-top:1px solid #ddd;background:#fff!important}
  ...
  /* the accordion's collapsed/expanded state is a screen-only affordance;
     print always shows all four case studies in full, regardless of which
     one was left open on screen */
  .work-item{border:none;border-top:1px solid #ddd;border-radius:0}
  .work-item:first-child{border-top:none}
  .w-chev{display:none}
  .work-item .w-body{-webkit-line-clamp:unset!important;line-clamp:unset!important;overflow:visible!important}
  .work-item summary,.exp-row{
...
<header id="nav">
  ...
  <div class="nav-act">
    ...
    <button class="theme-toggle" id="themeToggle" type="button" aria-label="Switch to light theme">
      <svg class="ico-sun" viewBox="0 0 24 24" fill="none" stroke-width="1.8" stroke-linecap="round" aria-hidden="true"><circle cx="12" cy="12" r="4.2"/><path d="M12 2.5v2.4M12 19.1v2.4M4.9 4.9l1.7 1.7M17.4 17.4l1.7 1.7M2.5 12h2.4M19.1 12h2.4M4.9 19.1l1.7-1.7M17.4 6.6l1.7-1.7"/></svg>
      <svg class="ico-moon" viewBox="0 0 24 24" fill="none" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M20 14.5A8.5 8.5 0 1 1 9.5 4a6.8 6.8 0 0 0 10.5 10.5Z"/></svg>
    </button>
...
<section id="work" class="section">
  ...
  <div class="wrap">
    ...
    <div class="work-list r">

      <details class="work-item" name="work-acc" open>
        <summary>
          <div class="w-metric">
            <p class="w-n">5,200</p>
          ...
          <span class="w-chev" aria-hidden="true"><svg viewBox="0 0 12 12" fill="none"><path d="M6 1v10M1 6h10" stroke="currentColor" stroke-width="1.4" stroke-linecap="round"/></svg></span>
        </summary>
      </details>

      <details class="work-item" name="work-acc">
        <summary>
          <div class="w-metric">
            <p class="w-n">450K</p>
          ...
          <div class="w-main">
            ...
            <p class="w-body">Designed and launched Union&rsquo;s in-house growth platform. 450,000 users onboarded in the first 72 hours through a funnel that cost nothing to acquire. The new testnet did 3&times; the transaction volume the old one had. It proved what I keep coming back to: distribution, built right, is itself a product feature.</p>
          </div>
          <span class="w-chev" aria-hidden="true"><svg viewBox="0 0 12 12" fill="none"><path d="M6 1v10M1 6h10" stroke="currentColor" stroke-width="1.4" stroke-linecap="round"/></svg></span>
        </summary>
      </details>
      ...
      <details class="work-item" name="work-acc">
        <summary>
          <div class="w-metric">
            <p class="w-n">28</p>
            <p class="w-unit">states<br>90 days</p>
          </div>
          <div class="w-main">
            <p class="w-tag"><a href="https://shardeum.org" target="_blank" rel="noopener">Shardeum</a><span class="sep"></span>Proof of Community</p>
            <h3 class="w-title">Every state in India in 90 days, on a $10K budget</h3>
            <p class="w-body">Shardeum needed presence in India with no regional team, no local recognition, and a lean budget. The plan was a 90-day tour reaching all 28 states, 53 activations in total, run single-handed from start to finish. Attendance was never the metric. Every stop ended with people creating wallets and deploying working dApps, which mattered to two partners for two different reasons: OKX counted the wallets, Shardeum counted the deployments, and one program served both scorecards. 20,000+ students and professionals reached, the curriculum adopted at 25+ universities, and a contributor network that kept growing after I left the country. The operating playbook became the template for Shardeum&rsquo;s global expansion.</p>
          </div>
          <span class="w-chev" aria-hidden="true"><svg viewBox="0 0 12 12" fill="none"><path d="M6 1v10M1 6h10" stroke="currentColor" stroke-width="1.4" stroke-linecap="round"/></svg></span>
        </summary>
      </details>
  ...
  <div class="wrap">
    ...
    <div class="cap-grid">
      <article class="cap-card r">
        ...
        <p class="cap-body">450,000 users in 72 hours. A newsletter at 51,000 subscribers and a 20% open rate. Partnerships with Aptos, Polygon, Arbitrum, OKX, and The Sandbox. Growth earned through community and partnerships outlasts paid acquisition and costs a fraction of it. I build the campaigns, the BD pipelines, the institutional and VC relationships, and the partner programs that generate that distribution.</p>
      </article>
      <article class="cap-card r">
        <p class="cap-i">02</p>
        <h3 class="cap-title">Complex programs, single-threaded ownership</h3>
        <p class="cap-kicker">Operations at scale</p>
        <p class="cap-body">I&rsquo;ve coordinated a 5,200-person program across 14 time zones, covered all 28 states of India in 90 days on $10,000, and produced an 800-person flagship conference in six weeks. The specifics change every time. What doesn&rsquo;t: I own the whole thing, I make it work, and I build the system so the next one doesn&rsquo;t depend on me.</p>
      </article>
      <article class="cap-card r">
        <p class="cap-i">03</p>
    ...
    <div class="about-grid">
      ...
      <aside class="about-aside r" aria-label="Career at a glance">
        ...
        <div class="aside-stats">
          ...
          <div class="a-stat"><span class="a-label">University curriculum partnerships</span><span class="a-val">45+</span></div>
    ...
    <div class="exp-list" id="expList">
      ...
      <article class="exp-row r">
        ...
        <div class="ex-body">
          ...
          <ul class="ex-bul">
            ...
            <li>Designed and launched Union&rsquo;s Quest platform: 50,000 users in the first 24 hours, 450,000 by 72 hours, with zero paid acquisition; the new testnet did 3&times; the transaction volume of the old one.</li>
      ...
      <article class="exp-row r">
        ...
        <div class="ex-body">
          ...
          <ul class="ex-bul">
            ...
            <li>Ran 400+ community activations across the region, measured in wallets created and dApps deployed rather than attendance, anchored by PoC Backpacking India: a 90-day tour across all 28 states, 53 activations on $10,000, reaching 20,000+ students and professionals, with the program adopted as blockchain curriculum at 25+ universities.</li>
      ...
      <article class="exp-row r" data-more>
        ...
        <div class="ex-body">
          ...
          <ul class="ex-bul">
            <li>Managed partner integrations for new platform partnerships, working directly with product and engineering to close the loop between partner and community feedback and the roadmap.</li>
            <li>Built the developer community from zero through technical content, contribution programs, and structured onboarding that shortened time to first contribution, and owned <a href="https://blog.gitopia.com" target="_blank" rel="noopener" class="ln">technical blogs</a> and <a href="https://www.youtube.com/channel/UCsAVjkAUnT5krP_e8HyFRHg" target="_blank" rel="noopener" class="ln">educational content</a> across all channels.</li>
          </ul>
        </div>
      </article>
      <article class="exp-row r" data-more>
...
<section id="vol" class="section">
  <div class="sec-num" aria-hidden="true">06</div>
  <div class="wrap">
    <div class="sec-head r">
...
<section id="praise" class="section">
  <div class="sec-num" aria-hidden="true">07</div>
  <div class="wrap">
    <div class="sec-head r">
...
<section id="contact" aria-label="Contact">
  <div class="ct-inner">
    ...
    <div class="ct-btns r">
      <a href="mailto:dhirloveneesh@gmail.com?subject=Role%20conversation" class="ct-primary">Email <span class="ar" aria-hidden="true">&rarr;</span></a>
...
(function(){
  ...
  /* ---- theme toggle ---- */
  /* data-theme is already set (dark by default, or a saved choice) by the
     inline script in <head>, which runs before first paint. This just wires
     the visible button up to flip it and persist the choice. */
  (function(){
    var btn = document.getElementById('themeToggle');
    var colorMeta = document.getElementById('themeColor');
    if (!btn) return;
    var COLORS = { dark: '#14110F', light: '#FFFFFF' };
    function label(theme){
      btn.setAttribute('aria-label', theme === 'dark' ? 'Switch to light theme' : 'Switch to dark theme');
    }
    function apply(theme){
      /* suppress transitions for the one frame the switch actually happens,
         see the .theme-switching CSS comment for why this exists */
      doc.classList.add('theme-switching');
      doc.setAttribute('data-theme', theme);
      if (colorMeta) colorMeta.setAttribute('content', COLORS[theme] || COLORS.dark);
      label(theme);
      try { localStorage.setItem('theme', theme); } catch(e){}
      /* setTimeout, not requestAnimationFrame: rAF can be throttled or
         never fire in a backgrounded/inactive tab (verified: a double-rAF
         version left this class stuck indefinitely in that exact scenario,
         permanently killing every transition on the page). A timer fires
         regardless of render state. Transitions are fully suppressed while
         this class is present, so the 60ms just needs to outlast one style
         recalc, not any transition duration, then normal hover/interaction
         transitions resume as soon as it's removed. */
      setTimeout(function(){ doc.classList.remove('theme-switching'); }, 60);
    }
    label(doc.getAttribute('data-theme') === 'light' ? 'light' : 'dark');
    btn.addEventListener('click', function(){
      apply(doc.getAttribute('data-theme') === 'dark' ? 'light' : 'dark');
    });
  })();

  /* Selected Work is native <details>/<summary> now (see CSS); no JS needed
     for the open/close behavior itself, the browser handles it. */
  ...
  /* body > footer, scoped deliberately: keeps matching only the page footer
     if a nested <footer> is ever reintroduced inside main content */
